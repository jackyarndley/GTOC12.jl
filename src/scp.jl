...
function process_solution_for_scp(
    ...
    t_nodes, 
    u_nodes, 
    coordinate_config::ModifiedEquinoctialConfig,
    thrust_config::ThrustConfig,
    objective_config::ObjectiveConfig
)
    x_start = copy(x_start_cartesian)
    ...
    x_start = cartesian_to_modified_equinoctial(x_start)
    x_final = cartesian_to_modified_equinoctial(x_final)
    x_start_injection = cartesian_to_modified_equinoctial(x_start_injection)

    x_nodes = propagate_spacecraft(x_start_injection, t_nodes; t_nodes, u_nodes, coordinate_config, thrust_config, objective_config)

    x_final_current = x_nodes[:, end]
    x_final_current_cartesian = modified_equinoctial_to_cartesian(x_final_current)

    return x_start, x_final, x_nodes, x_final_current, x_final_current_cartesian
end

function solve_scp(
    x_departure_cartesian::Vector{T},
    x_arrival_cartesian::Vector{T},
    t_nodes::Vector{T},
    u_nodes::Matrix{T},
    Δv_departure_injection_start::Vector{T},
    Δv_departure_injection_limit::T,
    Δv_arrival_injection_start::Vector{T},
    Δv_arrival_injection_limit::T,
    objective_config::ObjectiveConfig;
    ...
    if typeof(objective_config) == LoggedMassConfig
        u_nodes[1:3, :] .*= x_departure_cartesian[7]

        # Convert into the log mass for convex formulation
        x_departure_cartesian[7] = log(x_departure_cartesian[7])
        x_arrival_cartesian[7] = log(x_arrival_cartesian[7])
    end

    x_departure, x_arrival, x_nodes, x_arrival_current, x_arrival_current_cartesian = process_solution_for_scp(
        x_departure_cartesian, 
        x_arrival_cartesian, 
        Δv_departure_injection_start, 
        t_nodes, 
        u_nodes, 
        coordinate_config, 
        thrust_config,
    ...
    state_size = size(x_departure, 1)
    control_size = size(u_nodes, 1)
    ...
    # State at each node
    @variable(model, x[i=1:state_size, j=1:nodes], start = x_nodes[i, j])

    # Control at each node
    @variable(model, u[i=1:control_size, j=1:(nodes-1)], start = u_nodes[i, j])

    # Injection Δv for start and end nodes
    @variable(model, Δv_departure_injection[i=1:3], start = Δv_departure_injection_start[i])
    @variable(model, Δv_arrival_injection[i=1:3], start = Δv_arrival_injection_start[i])

    # SOC constraint for injection Δv
    @constraint(model, [Δv_departure_injection_limit; Δv_departure_injection[1:3]] in SecondOrderCone())
    @constraint(model, [Δv_arrival_injection_limit; Δv_arrival_injection[1:3]] in SecondOrderCone())

    # Ensure start and end node states match
    @constraint(model, x[1:6, 1] .== x_departure[1:6] .+ vcat([0, 0, 0], Δv_departure_injection))
    @constraint(model, x[1:6, nodes] .== x_arrival[1:6] .+ vcat([0, 0, 0], Δv_arrival_injection))

    if typeof(objective_config) == VelocityConfig
        timesteps = t_nodes[2:end] .- t_nodes[1:(end-1)]

        # SOC constraint for control
        @constraint(model, [i=1:(nodes-1)], [u[4, i]; u[1:3, i]] in SecondOrderCone())
        # @constraint(model, [i=1:(nodes-1)], [1.0; u[1:3, i]] in SecondOrderCone())

        # Limit on throttle at each node
        @constraint(model, u[4, :] .<= 1.0)

        # Objective is to minimize the Δv used
        @objective(model, Min, sum(u[4, :] .* timesteps))
    else
        m_ref = x_nodes[7, :]
        ...
        # Starting mass constraint
        @constraint(model, x[7, 1] == x_departure[7])

        # Objective is to maximise the final mass
        @objective(model, Max, x[7, nodes])
    end

    # Variables for SCP convergence
    dynamic_constraint = nothing
    thrust_mass_constraint = nothing
    maximum_error = 1e6
    ...
    for i in 1:scp_iterations
        ...
        stms = get_arc_state_transition(t_nodes, x_nodes, u_nodes, coordinate_config, thrust_config, objective_config)

        # Derived matrices from the STMs
        As = stms[:, 1:state_size, 1:(nodes-1)]
        Bs = stms[:, (state_size+1):end, 1:(nodes-1)]
        cs = x_nodes[:, 2:end] .- stack(As[:, :, j] * x_nodes[:, j] + Bs[:, :, j] * u_nodes[:, j] for j in 1:(nodes-1))

        # Delete the old linearisation
        if !isnothing(dynamic_constraint)
            for con in dynamic_constraint
                delete(model, con)
            end
        end

        if !isnothing(thrust_mass_constraint)
            for con in thrust_mass_constraint
                delete(model, con)
            end
        end

        # Dynamics propagated using STMs
        dynamic_constraint = @constraint(model,
            [i=1:(nodes-1)],
            x[:, i+1] .== As[:, :, i] * x[:, i] .+ Bs[:, :, i] * u[:, i] .+ cs[:, i]
        )

        # Constraint for thrust based on mass
        thrust_mass_constraint = if typeof(objective_config) == LoggedMassConfig
            @constraint(model, 
                [i=1:(nodes-1)],
                u[4, i] <= exp.(-m_ref[i]).*(1.0 .- x[7, i] .+ m_ref[i])
            )
        end
        ...
        maximum_error = maximum(abs.(x_arrival_current_cartesian[1:6] .- x_arrival_cartesian[1:6] .- vcat([0, 0, 0], value.(Δv_arrival_injection))))

        if typeof(objective_config) == LoggedMassConfig
            maximum_error = maximum([maximum_error, abs(m_ref[end] - x_nodes[7, end])])

            m_ref = x_nodes[7, :]
        end

        if display_output
            @printf "\n%2i  %10.6e  %10.6f  %7.6f  %7.6f  %s" i maximum_error exp(objective_value(model))*m_scale t2-t1 time()-t2 termination_status(model)
        end
    ...
    if typeof(objective_config) == LoggedMassConfig
        ...
        x_nodes[7, :] = exp.(x_nodes[7, :])
    end
    
    return x_nodes, u_nodes, value.(Δv_departure_injection), value.(Δv_arrival_injection), termination_status(model), maximum_error
end
        ...
        for n in active_parts
            ...
            for k in bad_dynamic_segments[n]
                ...
                trust_region_dynamic_constraint[n][k] = @constraint(model[n],
                    [i=1:(nodes-1)],
                    # -0.05 .<= x[k][1:6, i] .- x_nodes[k][1:6, i] .<= 0.05
